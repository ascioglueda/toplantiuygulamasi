...
class MeetingApp:
    def __init__(self, master):
        ...
        self.button_show_available_dates = tk.Button(master, text="Show Available Dates", command=self.show_available_dates)
        self.button_register_participation = tk.Button(master, text="Register Participation", command=self.register_participation)
        self.button_add_to_google_calendar = tk.Button(master, text="Add to Google Calendar", command=self.add_to_google_calendar)
    ...
    def show_available_dates(self):
        code = self.entry_code.get()
        if code:
            available_dates = self.scheduler.show_available_dates(code)
            if available_dates:
